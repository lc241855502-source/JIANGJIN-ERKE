...
st.title("📊 门店提成一键计算系统")
st.caption("自动多备用金合并LS，完整复刻Excel绩效提成公式")
st.divider()

# 文件上传区域
col1, col2 = st.columns(2)
with col1:
    business_upload = st.file_uploader("① 业务总表xlsx", type=["xlsx"], help="包含26.05完成情况、销售明细、所有备用金Sheet")
with col2:
...
if st.button("🚀 开始计算", type="primary", disabled=not can_run):
    ...
    if business_upload is None or staff_upload is None:
        ...
        try:
            ...
            st.info("排查方向：1、Excel格式损坏；2、Sheet名称不匹配；3、配置表列缺失；4、数字存在非法字符")

# 使用说明
with st.expander("使用说明"):
    st.markdown("""
1. 业务文件自动识别全部名称含「备用金」工作表，合并统计LS扣款
2. 产品类型自动根据品牌匹配，无需手动填写
3. 销售Y/Z列1:1复刻原Excel全部特殊订单规则
