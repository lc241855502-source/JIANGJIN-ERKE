...
def calc_staff_perf(staff_data, store_data):
    ...
    if beh_raw > 2:
        ...
        beh_score = beh_raw
    perf_rate = round(min(completion, 1.0), 3) if completion >= 0.7 else 0.0
...
def run_calculation(business_file, config_file, store_sheet="26.05完成情况", sales_sheet="26.05销售明细"):
    ...
    if miss_staff:
        raise ValueError(f"人员表缺失列：{miss_staff}")
    ...
    for _, s in df_staff.iterrows():
        ...
        if is_clear:
            res_rows.append({"姓名":str(s["姓名"]),"部门名称":str(s["部门名称"]),"绩效小计":0.0,"提成总计":0.0,"调差总计":0.0,"备注":remark})
            continue
...
def generate_config():
    cols = ["姓名","工号","部门名称","部门代码","职位","备注","是否有提成资格","绩效设定","行为绩效","库存机奖励","异常补差","转介绍","个人提成调整"]
