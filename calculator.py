...
def smart_read_excel(file, sheet_name, required_keywords):
    """优化表头识别：兼容多行合并标题，自动定位真实表头行"""
    df_raw = pd.read_excel(file, sheet_name=sheet_name, header=None)
    header_row = 0
    max_hit = 0
    # 扫描前15行，找到关键词匹配最多的行作为表头
...
def calc_z_row(row):
    ...
    if pd.notna(row.get("LS分摊比例")):
        ls_ratio = float(row.get("LS分摊比例", 0.0))
    elif pd.notna(row.get("LS")):
        ls_ratio = float(row.get("LS", 0.0))
